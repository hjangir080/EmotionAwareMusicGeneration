...
class MusicGenGenerator:
    """
    Open source music generation using Meta's AudioCraft (MusicGen)
    """
    def __init__(self, model_size="large"):
        """
        Initialize MusicGen with specified model size
        model_size: "small", "medium", "large", or "melody" (for melody conditioning)
        """
    ...
    def generate_music(self, prompt, duration=10, num_samples=1, seed=None):
        """
        ...
        Returns:
            dict: Generated audio samples and file paths
        """
        if seed is not None:
        ...
        # Save samples to files and collect paths
        file_paths = []
        for i, audio in enumerate(wav):
            audio_numpy = audio.cpu().numpy()
            filename = f"generated_music_sample_{i+1}.wav"
            torchaudio.save(filename, audio, sample_rate=32000)
...
class SunoAIGenerator:
    """
    Music generation using Suno AI's API - considered one of the best in 2025
    """
    def __init__(self, api_key=None):
        self.api_key = api_key or os.getenv("SUNO_API_KEY")
        if not self.api_key:
            raise ValueError("Suno API key is required. Set it as SUNO_API_KEY environment variable or pass directly.")
        
        self.base_url = "https://api.suno.ai/v1/generation"
        self.headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json"
        }
    
    def generate_music(self, prompt, style=None, duration=30, seed=None):
        """
        Generate music from a text prompt using Suno AI
        
        Args:
            prompt (str): Detailed description of the music to generate
            style (str, optional): Musical style tag
            duration (int): Duration in seconds (max allowed by Suno)
            seed (int, optional): Random seed for reproducibility
            
        Returns:
            dict: Response containing audio data and generation info
        """
        data = {
            "prompt": prompt,
            "duration": duration
        ...
        try:
            ...
            if audio_url:
                ...
                # Save to file
                filename = f"generated_music_{result.get('id', 'unknown')}.mp3"
...
def generate_music_complete(text, emotion_extractor, emotion_mapper, generator_type="suno", **generator_kwargs):
    """
    Full pipeline: analyze text emotions → map to music parameters → generate music
    
    Args:
        text (str): Input text to analyze and convert to music
        emotion_extractor: EmotionExtractionModule instance
        emotion_mapper: EmotionToMusicMapper instance
        generator_type (str): "suno" or "musicgen"
        **generator_kwargs: Additional arguments for the generator
        
    Returns:
        dict: Result dictionary with all information
    """
    # Step 1: Extract emotions from text
    emotion_info = emotion_extractor(text)
    emotion_scores = emotion_info["emotion_scores"]
    emotion_categories = emotion_info["emotion_categories"]
    ...
    # Step 2: Get emotional arc for longer texts
    if len(text.split()) > 100:
        arc_info = emotion_extractor.extract_emotional_arc(text)
        emotional_arc = arc_info["emotional_arc"]
        segments = arc_info["segments"]
    ...
    # Step 3: Generate MusicLM-compatible prompts from emotions
    if emotional_arc is not None:
        prompts = emotion_mapper.generate_musiclm_prompt(emotional_arc, emotion_categories)
        print(f"Generated {len(prompts)} segment prompts from emotional arc")
    else:
    ...
    # Step 4: Initialize the appropriate music generator
    if generator_type.lower() == "suno":
        generator = SunoAIGenerator(**generator_kwargs)
    else:  # Default to MusicGen
        generator = MusicGenGenerator(**generator_kwargs)
    
    # Step 5: Generate music for each prompt (or just the first one for simplicity)
    # For simplicity, we'll just use the first prompt, but you could generate for each segment
    main_prompt = prompts[0]
    print(f"Generating music with prompt: {main_prompt[:100]}...")
    
    # Extract style from prompt if possible
    style = None
    if "in " in main_prompt and " key" in main_prompt:
        style_text = main_prompt.split("in ")[1].split(" key")[0]
        if len(style_text.split()) <= 2:  # Simple style extraction
            style = style_text
    
    # Generate the music
    music_result = generator.generate_music(
        prompt=main_prompt,
        style=style
    )
    ...
    return {
        ...
        "audio_file": music_result.get("local_file") or music_result.get("file_paths", [None])[0]
    }
...
if __name__ == "__main__":
    ...
    # Alternative: Use MusicGen (open source, runs locally)
    result = generate_music_complete(
        literary_text, 
        emotion_extractor, 
        emotion_mapper,
        generator_type="musicgen",
        model_size="large"
    )